...
| **主题同步**（v1.9 起侧边栏叫「主题」） | 本地 `Themes` 目录与 NAS 互传：上传 / 下载 / 双向，带预演与冲突留档。v1.9 起改成**卡片勾选式**：本机与 NAS 上的主题各一张卡，勾谁搬谁 |
| **自更新** | 打开 Playnite 自动检查插件更新，国内 / 国外自动换源，慢网络有兜底 |
| **侧边栏控制台** | 左侧边栏一格管完：概览统计 / 主题 / 仓库 / 任务 / 设置。明暗三档（跟随 Playnite / 常亮 / 常暗），左栏标题右边一个小圆点报连通状态（点它立刻重测） |
| **仓库**（v1.8.1 起） | 本机库里的游戏与 NAS 上独有的条目**铺在同一张卡片墙**上，每个游戏标出「已归档 / 未归档 / 只在 NAS」与体积；未传的可直接归档、只在 NAS 的可以取回或删除。判定按 Playnite 游戏 ID → 库内 ID → 安装目录名 → 库内标识直对条目 Id → 游戏名字**五级** |
| **任务队列**（v1.9） | 归档 / 取回 / 卸载 / 主题同步都**不再弹模态进度框**：排进串行队列在后台跑，页面底部一条聚合进度条（像 Steam 那样），专门的「任务」页可以逐条看进度与失败原因，也能整体取消 |
...
| `tools/VaultSelfTest/` | **插件侧自检**：本地假 GitHub / 假 Gitee，把自动更新与自动刷新整条链路真跑一遍，外加改名迁移、主题同步、云存档引擎与嗅探，以及 v1.8 的界面部件 / 整页构造 / 卡片对账 / 删除入口唯一，和 v1.9 的五级命中键 / 主题归并 / 传输队列（**345 项断言**） | C# / .NET Framework 4.6.2 |
...
**退出码 0 = 345 项断言全过**，1 = 有失败项。失败项会给出实际值（例如
