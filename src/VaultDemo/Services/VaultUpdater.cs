...
namespace VaultDemo.Services
{
    ...
    public class ReleaseInfo
    {
        ...
        /// <summary>挑出插件包：VaultDemo-x.y.z.zip。</summary>
        public ReleaseAsset PickPluginAsset()
        {
            ...
            // 最精确：VaultDemo-1.5.0.zip
            var exact = zip.FirstOrDefault(a =>
                a.Name.StartsWith("VaultDemo-", StringComparison.OrdinalIgnoreCase));
            if (exact != null)
            {
                return exact;
            }

            // 宽松一点：名字里带 VaultDemo 的 zip
            return zip.FirstOrDefault(a =>
                a.Name.IndexOf("VaultDemo", StringComparison.OrdinalIgnoreCase) >= 0);
        }
    }
    ...
    ///
    /// 三件事必须说清楚：
    /// 1) **为什么要外部脚本**：VaultDemo.dll 正被 Playnite 的进程加载着（Windows 文件锁），
    ...
    public class VaultUpdater
    {
        ...
        /// <summary>插件安装目录（VaultDemo.dll 所在目录）。</summary>
            ...
            ProxyMode proxy = ProxyMode.Direct)
        {
            return System.Threading.Tasks.Task.Run(() =>
            {
                ...
                if (probe.Asset == null)
                {
                    probe.Error = "release 里没有 VaultDemo-*.zip 附件";
                }
                return probe;
            });
        }
            ...
            Func<bool> shouldCancel)
        {
            ...
            var zipPath = Path.Combine(UpdateRoot, "VaultDemo-" + version + ".zip");
        ...
        public StagedUpdate Stage(string zipPath, string expectedVersion)
        {
            ...
            using (var archive = ZipFile.OpenRead(zipPath))
            {
                foreach (var entry in archive.Entries)
                {
                    ...
                    // 发布包里是 VaultDemo_<guid>/<文件>，只取一层目录里的文件。
                    // 更深的层级不属于插件根目录，直接忽略。
                    if (DepthOf(name) > 1)
                    {
                        continue;
            ...
            var dll = Path.Combine(StagingDir, "VaultDemo.dll");
            var yaml = Path.Combine(StagingDir, "extension.yaml");
            if (!File.Exists(dll) || !File.Exists(yaml))
            {
                throw new InvalidDataException("更新包里缺少 VaultDemo.dll 或 extension.yaml");
            }

            if (!LooksLikeManagedAssembly(dll))
            {
                throw new InvalidDataException("VaultDemo.dll 不是有效的 PE 文件");
            }
            ...
            if (!string.IsNullOrEmpty(id) && id.IndexOf("VaultDemo_", StringComparison.OrdinalIgnoreCase) != 0)
            {
                throw new InvalidDataException("更新包的插件 Id 不是 VaultDemo_ 开头：" + id);
            }
        ...
        /// <summary>路径深度：顶层文件 0，<c>VaultDemo_x/VaultDemo.dll</c> 是 1。</summary>
        ...
        public StagedUpdate PendingStaged()
        {
            ...
            {
                var yaml = Path.Combine(StagingDir, "extension.yaml");
                if (!File.Exists(yaml) || !File.Exists(Path.Combine(StagingDir, "VaultDemo.dll")))
                {
                    return null;
                }
            ...
            string targetDir, string launchExe, string launchArgs, string logPath)
        {
            ...
            builder.AppendLine("rem 由 VaultDemo 插件自动生成 —— 等 Playnite 退出后替换插件文件并重新拉起。");
